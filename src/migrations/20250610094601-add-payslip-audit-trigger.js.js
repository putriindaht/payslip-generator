...
module.exports = {
  async up (queryInterface, Sequelize) {
      ...
      CREATE OR REPLACE FUNCTION log_payslip_audit()
      RETURNS TRIGGER AS $$
      DECLARE
    ...
    await queryInterface.sequelize.query(`
      DROP TRIGGER IF EXISTS trg_log_payslip_audit ON payslips;
      CREATE TRIGGER trg_log_payslip_audit
      AFTER INSERT OR UPDATE OR DELETE ON payslips
      FOR EACH ROW
      EXECUTE FUNCTION log_payslip_audit();
    `);
  },
  ...
  async down (queryInterface, Sequelize) {
    await queryInterface.sequelize.query(`
      DROP TRIGGER IF EXISTS trg_log_payslip_audit ON payslips;
      DROP FUNCTION IF EXISTS log_payslip_audit();
    `);
  }
};
